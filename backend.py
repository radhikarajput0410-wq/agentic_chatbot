...
# ================= Conversation metadata (custom titles) =================
# Titles are normally auto-derived from a thread's first human message
# (see get_last_human_message / _shorten_title in app.py). This table
# lets a user override that auto-derived title with one of their own
# choosing, and have the override persist across refreshes, browser
# restarts, and Streamlit reruns -- exactly like renaming a chat in
# ChatGPT/Claude. A thread with no row here simply falls back to the
# auto-derived title, so nothing has to migrate for existing threads.
conn.execute(
    """
...
def set_conversation_title(thread_id, title):
    """
    Persist a user-chosen (renamed) title for a conversation thread.
    Overrides the auto-derived "first message" title going forward,
    for this browser session and any future one that loads this thread.
    """
    ...
    conn.execute(
        "INSERT INTO conversation_meta (thread_id, custom_title, updated_at) "
        "VALUES (?, ?, ?) "
        "ON CONFLICT(thread_id) DO UPDATE SET "
        "custom_title = excluded.custom_title, updated_at = excluded.updated_at",
        (thread_id, cleaned_title, datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()
...
def get_conversation_title(thread_id):
    """
    Return the user-set custom title for a thread, or None if the
    thread has never been renamed (in which case the caller should
    fall back to deriving a title from the first message).
    """
    if not thread_id:
        return None
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT custom_title FROM conversation_meta WHERE thread_id = ?",
        (thread_id,),
    )
    row = cursor.fetchone()
